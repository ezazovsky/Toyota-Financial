...
export class CarService {
  ...
  static getTrimsForModel(model: string): string[] {
    const trims = TOYOTA_MODELS.filter(
      car => car.model.toLowerCase() === model.toLowerCase()
    ).map(car => car.trim)
  ...
  static searchCars(criteria: {
    ...
    return TOYOTA_MODELS.filter(car => {
      ...
      if (
        criteria.bodyStyle &&
        car.specifications.bodyStyle.toLowerCase() !==
          criteria.bodyStyle.toLowerCase()
      ) {
        return false
      }
      if (
        ...
        car.specifications.fuelType.toLowerCase() !==
          criteria.fuelType.toLowerCase()
      ) {
        return false
      }
      if (
        criteria.drivetrain &&
        car.specifications.drivetrain.toLowerCase() !==
          criteria.drivetrain.toLowerCase()
      ) {
        return false
      }
      return true
